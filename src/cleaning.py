...
CHUNK_SIZE = 100_000  # rows per chunk for large file processing
...
# Required: consistent snake_case names, common alias resolution
# ==============================================================================
...
def clean_file_chunked(name: str) -> None:
    """Clean a large JSON file in chunks to avoid loading it all into memory.

    Reads CHUNK_SIZE rows at a time, cleans each chunk, and appends to the
    output CSV incrementally. No full-file dedup is performed — within-chunk
    dedup only. This is acceptable for review/user files where IDs are unique.

    Note: imputation uses per-chunk statistics (not global), which is a
    known tradeoff of chunked processing vs. accuracy.
    """
